...
class AdminLogTest extends TestCase
{
    ...
    /**
     * Журнал переживает удаление автора.
     *
     * Вопрос «кто это сделал» задают как раз после увольнения, и ответ
     * «пользователь #17, которого больше нет» на него не отвечает.
     *
     * Отдельно проверяется, что само удаление тоже записалось. С внешним
     * ключом на автора эта запись не вставлялась вовсе — она ссылается
     * на строку, которой в этот момент уже нет, — и на PostgreSQL
     * отменяла всю транзакцию заодно.
     */
    #[Test]
