...
class BillingController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('cabinet/Billing', [
            ...
            /*
             * Показывать ли форму ввода промокода. Прячется она только
             * после активированного кода: скидочные коды доступны
             * и платившим компаниям, поэтому оплата в прошлом форму
             * больше не убирает.
             */
     ...
     * Активация промокода.
     *
     * Код на бесплатный период выдаёт тариф сразу, без счёта. Скидочный
     * код возвращает из сервиса счёт на остаток цены — с него покупатель
     * уезжает на платёжную страницу Uzum, как при обычной покупке;
     * если онлайн-касса выключена, счёт оплачивается по реквизитам.
     *
    ...
    public function promo(Request $request): HttpResponse
    {
        ...
        try {
            $result = $promos->redeem($data['promo_code'], $company, $request->user());
             ...
             * активировать код с другой вкладки), форма со следующей
             * загрузкой исчезнет вместе с сообщением, и нажатие
        ...
        if ($result instanceof Payment) {
            // Остаток цены оплачивается онлайн — уводим на кассу сразу.
            // Без кассы счёт ждёт в «Счетах к оплате» вместе с реквизитами
            if ($this->checkoutEnabled()) {
                return $this->checkout($result);
            }

            $percent = $result->promoCode?->discount_percent;
